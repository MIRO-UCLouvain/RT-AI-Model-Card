...
APPENDIX_HINT = (
    ...
    "For fields that only accept a single image, upload the additional "
    "figures here.\n\n"
...
# Extend this dict to add sections or subsections without touching the UI.
...
def _effective_subsection(file_data: dict[str, str]) -> str:
    """Return the subsection label, resolving 'Other' to the custom text."""
...
def _render_uploaded_row(
    ...
    """Display a numbered appendix item with classification and preview.
    ...
    _render_section_selectors(original_name, file_data)
    _render_item_preview(file_path, original_name)

    section_divider()


def _render_subsection_selector(
    original_name: str,
    file_data: dict[str, str],
    new_section: str,
) -> None:
    """Render the subsection selector for an already classified item."""
    entry = st.session_state.appendix_uploads[original_name]
    subsection_options = [
        _SUBSECTION_PLACEHOLDER,
        *SECTION_SUBSECTIONS[new_section],
    ]

    stored_sub = file_data.get("subsection") or ""
    # Reset the subsection when the section changed and it's no longer valid
    ...
    entry["subsection"] = new_subsection

    # Free-text input when "Other" is chosen
    if new_subsection == _OTHER:
        entry["subsection_custom"] = st.text_input(
            ...
            help="This label appears next to the section name.",
        )
    else:
        entry["subsection_custom"] = ""


def _render_section_selectors(
    original_name: str,
    file_data: dict[str, str],
) -> None:
    """Render the section / subsection classification for an appendix item."""
    entry = st.session_state.appendix_uploads[original_name]
    section = file_data.get("section") or ""
    ...
    if new_section and new_section in SECTION_SUBSECTIONS:
        _render_subsection_selector(original_name, file_data, new_section)
    else:
...
def _render_item_preview(file_path: str, original_name: str) -> None:
    """Render an inline image preview, or an expander for other file types."""
